...
<template>
  ...
  <button class="right-0 z-10 fixed center <xl:hidden" @click="toggleTheme()">
    <i style="font-size:28px" class=" p-5 iconfont " :class="theme === 'light' ? 'icon-taiyang' : 'icon-yueliang'"></i>
  </button>
  ...
  <div class="flex justify-center ">
    <div class="flex justify-center items-center xl:overflow-hidden xl:justify-start <xl:flex-col">
      <div v-show="notMobblePage"
        class="flex flex-col w-screen-xs p-4 z-3 items-center hideBar xl:h-200 xl:overflow-y-auto ">
        <leftBar />
      </div>
      <div class="h-auto shadow-md p-5 w-100  z-3 hideBar xl:h-200 xl:w-screen-lg xl:overflow-y-auto">
        <router-view />
      </div>
    </div>

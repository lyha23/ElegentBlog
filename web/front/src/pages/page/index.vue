...
<template>
  <div class="xl:flex-wrap xl:flex xl:w-220 xl:justify-around">
    <cover v-for="item in state.list" :item="item"></cover>
  </div>
  <footer class="flex mt-5">
    <nav class="flex nav">
      <router-link
        :to="`/page?page=${state.pagination.page - 1}`"
      ...
      <div class="flex" v-if="state.pagination.totalPage < 10">
        <router-link
          v-for="(item, _) in state.pagination.totalPage"
          :to="`/page?page=${item}`"
          class="navItem"
      ...
      <div v-else class="flex">
        <router-link
          :to="`/page/?page=1`"
          class="navItem"
          :class="1 == Number(Route.query.page) ? 'selectedNav' : ''"
        >
          1
        </router-link>
        <router-link
          v-show="[1, 2, 3].indexOf(Number(state.pagination.page)) != -1"
        ...
        <span v-show="Number(Route.query.page) > 3" class="navItem"> ... </span>
        <router-link
          v-show="
            [
              1,
              2,
              3,
              state.pagination.totalPage - 2,
              state.pagination.totalPage - 1,
              state.pagination.totalPage - 2,
              state.pagination.totalPage,
            ].indexOf(Number(Route.query.page)) == -1
          "
          v-for="(item, index) in [
        ...
        <router-link
          v-show="
            [
              ...
              state.pagination.totalPage - 1,
              state.pagination.totalPage,
            ].indexOf(Number(state.pagination.page)) != -1
          "
        ...
        <router-link
          :to="`/page/?page=${state.pagination.totalPage}`"
          class="navItem"
      ...
      <router-link
        :to="`/page/?page=${state.pagination.page + 1}`"
        v-show="state.pagination.page < state.pagination.totalPage"
        class="navItem"
      >
